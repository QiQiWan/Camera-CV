...
def _iter_mvs_runtime_dirs():
    """Return candidate MVS runtime directories for Windows DLL loading."""
    candidates = []

    env_keys = [
        'HIKROBOT_MVS_RUNTIME',
        'MVS_RUNTIME',
        'HIKROBOT_MVS_PATH',
        'MVS_PATH',
    ]
...
def _register_dll_dirs(dll_dirs):
    registered = []
...
def load_system_dll(dll_name):
    """Load MvCameraControl.dll robustly on Windows with MVS runtime search."""
    if os.name != 'nt':
        return _MvSdkUnavailable('MvCameraControl.dll can only be loaded on Windows.')

    runtime_dirs = list(_iter_mvs_runtime_dirs())
    _register_dll_dirs(runtime_dirs)

    attempted = []

    # 1) Prefer full-path loads from known runtime directories.
    for runtime_dir in runtime_dirs:
        ...
        try:
            if 'winmode' in WinDLL.__init__.__code__.co_varnames:
                return WinDLL(str(dll_path), winmode=0)
            return WinDLL(str(dll_path))
        except Exception as exc:
            attempted.append(f'{dll_path} -> {exc}')

    # 2) Fall back to bare name after DLL directories were registered.
    attempted.append(dll_name)
    try:
        if 'winmode' in WinDLL.__init__.__code__.co_varnames:
            return WinDLL(dll_name, winmode=0)
        return WinDLL(dll_name)
    except Exception as exc:
        attempted.append(f'{dll_name} -> {exc}')

    details = '; '.join(attempted)
    message = (
        f'加载系统 DLL {dll_name} 失败。已尝试路径: {details}. '
        '请确认已安装 64 位 MVS，并且运行时目录存在。常见路径: '
        r'C:\Program Files (x86)\Common Files\MVS\Runtime\Win64_x64'
    )
    print(message)
    return _MvSdkUnavailable(message)


MvCamCtrldll = load_system_dll('MvCameraControl.dll')
