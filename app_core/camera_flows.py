...
class CameraFlowMixin:
        ...
        while self.is_running_a:
            try:
                ...
                if frame is not None and getattr(frame, 'size', 0) > 0:
                    ...
                    empty_count += 1
                    if empty_count in (50, int(getattr(self.config, 'camera_a_empty_recover_threshold', 150) or 150)):
                        try:
                            mv_stats = self.cameraA.get_preview_stats() if getattr(self, 'cameraA', None) is not None else {}
                        except Exception:
                            mv_stats = {}
                        pixel_name = mv_stats.get('pixel_type_name', '') or 'Unknown'
                        grab_fps = float(mv_stats.get('grab_fps', 0.0) or 0.0)
                        display_fps = float(mv_stats.get('display_fps', 0.0) or 0.0)
                        last_error = mv_stats.get('last_error', '') or getattr(self.cameraA, 'last_error', '') or ''
                        print(f'[Camera A] preview frame is still empty; grab_fps={grab_fps:.1f}, display_fps={display_fps:.1f}, pixel={pixel_name}, last_error={last_error}')
                        ...
                        recover_threshold = int(getattr(self.config, 'camera_a_empty_recover_threshold', 150) or 150)
                        if empty_count >= recover_threshold and bool(getattr(self.config, 'camera_a_auto_recover_preview', True)):
                            now_ts = time.time()
                            cooldown = max(1.0, float(getattr(self.config, 'camera_a_preview_recover_cooldown_s', 5.0) or 5.0))
                            if (now_ts - float(getattr(self, '_camera_a_preview_recover_last_ts', 0.0) or 0.0)) >= cooldown:
                                self._camera_a_preview_recover_last_ts = now_ts
                                try:
                                    ok, info = self.cameraA.recover_preview_stream()
                                    print(f'[Camera A] preview auto-recover: ok={ok} info={info}')
                                    if ok:
                                        self.update_model_status_label('工业相机A预览空帧，已自动重启取流。')
                                except Exception as exc:
                                    print(f'[Camera A] preview auto-recover failed: {exc}')
                    time.sleep(0.01)
            except Exception as exc:
    ...
    def capture_image_a(self):
        ...
        try:
            ...
            if frame is None or getattr(frame, 'size', 0) == 0:
                for _tmp_path in temp_picture_candidates:
                    if self.wait_for_file_ready(_tmp_path, timeout=min(2.5, capture_timeout)):
                        pixmap = QPixmap(_tmp_path)
    ...
    def video_loop_b(self, session_id=None):
        ...
        stall_timeout_s = max(2.0, float(getattr(self.config, 'camera_b_stall_timeout_s', 6.0)))
        fail_threshold = max(10, int(getattr(self.config, 'camera_b_read_fail_threshold', 120)))
        retry_burst = max(0, int(getattr(self.config, 'camera_b_read_retry_burst', 2) or 0))
        retry_delay_s = max(0.0, float(getattr(self.config, 'camera_b_retry_read_delay_ms', 6) or 0.0) / 1000.0)
        active_session = int(session_id if session_id is not None else getattr(self, 'camera_b_session_id', 0) or 0)
        while self.is_running_b:
            if active_session != int(getattr(self, 'camera_b_session_id', 0) or 0):
                break
            loop_start = time.perf_counter()
            if self._camera_b_is_paused():
                consecutive_failures = 0
                self.camera_b_read_failures = 0
                time.sleep(0.01)
                continue
            try:
                ...
                if safe_frame is not None:
                    ...
                    if time.time() < float(getattr(self, 'camera_b_resume_guard_until', 0.0) or 0.0):
                        time.sleep(0.01)
                        continue
                    consecutive_failures += 1
                    self.camera_b_read_failures = consecutive_failures
                    if consecutive_failures in (10, 30) or consecutive_failures % 60 == 0:
                        print(f'[Camera B] read failed x{consecutive_failures} on backend {self.backend_name(self.current_camera_b_backend)}')
                    if active_session != int(getattr(self, 'camera_b_session_id', 0) or 0):
                        break
                    if consecutive_failures >= fail_threshold or ((time.time() - float(getattr(self, 'camera_b_last_frame_ts', 0.0) or 0.0)) >= stall_timeout_s and consecutive_failures >= 10):
                        if not self._camera_b_is_paused():
                            self._schedule_camera_b_reconnect(f'read failed x{consecutive_failures}')
                        break
                    time.sleep(0.01)
            except Exception as exc:
                if time.time() < float(getattr(self, 'camera_b_resume_guard_until', 0.0) or 0.0):
                ...
                if consecutive_failures >= fail_threshold:
                    if not self._camera_b_is_paused():
                        self._schedule_camera_b_reconnect(f'video loop error: {exc}')
                    break
                time.sleep(0.02)
